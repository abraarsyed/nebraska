...
var sortFilterMap = map[int]string{
	id:                  "id",
	ip:                  "ip",
...
func sanitizeSortFilterParams(sortFilter string) string {
	...
	if value, ok := sortFilterMap[sortFilterNumericValue]; ok {
		return value
	}
...
func (api *API) getInstanceFromApp(instanceIDOfApp string) (*Instance, error) {
	query, _, err := goqu.From("instance").Where(goqu.C("id").Eq(instanceIDOfApp)).ToSQL()
	if err != nil {
		return nil, err
	}
	var instance Instance
...
func (api *API) GetInstances(p InstancesQueryParams, duration string) (InstancesWithTotal, error) {
	...
	if existsInInstanceAppTable {
		...
		for _, application := range applications {
			var instance *Instance
			instance, err = api.getInstanceFromApp(application.InstanceID)
			if err != nil {
				return InstancesWithTotal{}, err
			}
